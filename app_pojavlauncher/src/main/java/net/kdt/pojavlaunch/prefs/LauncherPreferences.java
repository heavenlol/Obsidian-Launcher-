...
public class LauncherPreferences {
    ...
    public static void loadPreferences(Context ctx) {
        ...
        if ("auto".equalsIgnoreCase(PREF_RENDERER)) {
            try {
                if (GLInfoUtils.getGlInfo().isAdreno() && SDK_INT >= 29) {
                    PREF_RENDERER = "freedreno";
                } else {
                    PREF_RENDERER = "zink";
                }
            } catch (Exception e) {
        ...
        for (String arg : JREUtils.parseJavaArguments(PREF_CUSTOM_JAVA_ARGS)) {
            if (arg.startsWith(argLwjglLibname)) {
                DEFAULT_PREF.edit().putString("javaArgs", PREF_CUSTOM_JAVA_ARGS.replace(arg, "")).apply();
            }
        }
